//------------------------------------------------------------------------------
// <auto-generated>
//    這個程式碼是由範本產生。
//
//    對這個檔案進行手動變更可能導致您的應用程式產生未預期的行為。
//    如果重新產生程式碼，將會覆寫對這個檔案的手動變更。
// </auto-generated>
//------------------------------------------------------------------------------

namespace OrixModel
{
    using System;
    using System.Collections.Generic;
    
    public partial class OR_OBJ_DEPR_SET
    {
        public string OBJ_CODE { get; set; }
        public decimal QTY { get; set; }
        public string UNIT { get; set; }
        public string GET_TIME { get; set; }
        public decimal GET_AMT { get; set; }
        public decimal OBJ_SUR { get; set; }
        public decimal TOL_MTH { get; set; }
        public string DEPR_YM_FR { get; set; }
        public string DEPR_YM_TO { get; set; }
        public string DEPR_CODE { get; set; }
        public string DEPR_MTHD { get; set; }
        public decimal DEPR_RATE { get; set; }
        public string ADD_USER_ID { get; set; }
        public string ADD_DATE { get; set; }
        public string ADD_TIME { get; set; }
        public string LAST_UPD_USER_ID { get; set; }
        public string LAST_UPD_DATE { get; set; }
        public string LAST_UPD_TIME { get; set; }
    }
}
