<%@ Page  Language="C#" EnableEventValidation = "false" AutoEventWireup="true" MasterPageFile="~/Pattern/query.Master" CodeBehind="WA060.aspx.cs" Inherits="OrixMvc.WA060" %>
...
<asp:Content ContentPlaceHolderID="queryBody" ID="myQueryBody" runat="server">   
    ...
    <td class="fixCol"><%# Eval("APLY_NO")%><asp:HiddenField runat="server" ID="hiddenAPLY_NO" Value='<%# Eval("APLY_NO")%>' /><asp:HiddenField runat="server" ID="hiddenCUST_NO" Value='<%# Eval("CUST_NO")%>' /><asp:HiddenField runat="server" ID="hiddenCUR_STS" Value='<%# Eval("STSCODE").ToString().Trim()+","+Eval("CUR_STS").ToString().Trim()%>' /> </td>
    <td><%# Eval("CUST_NO")%></td>
